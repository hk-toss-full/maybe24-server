...
public class UserServiceImpl implements UserService {
    ...
    public String login(LoginRequest loginRequest) {
        ...
        if(loginUser.isEmpty())
            throw new CustomException(401, "해당 아이디를 가진 사용자가 없습니다");
        User user = loginUser.get();
        if(!passwordEncoder.matches(
                loginRequest.password(), user.getPassword()))
            throw new CustomException(401, "아이디와 비밀번호가 일치하지 않습니다");
    ...
    public void register(RegisterRequest request) {
        ...
        if(byUserId.isPresent()) throw new CustomException(409, "이미 등록된 아이디입니다");
